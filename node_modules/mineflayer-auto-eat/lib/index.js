...
module.exports = function (bot, options) {
    ...
    function eat(callback, manual = false) {
        ...
        if (bot.autoEat.isEating)
            return callback(Error('Already eating'));
        ...
        (async () => {
            ...
            bot.autoEat.isEating = false;
            callback(null);
            if (bot.food < bot.autoEat.options.startAt + 1)
                eat();
        })();
    }
    bot.on('health', () => {
        if (bot.autoEat.disabled || bot.food >= bot.autoEat.options.startAt)
            return;
        else if (bot.pathfinder && (bot.pathfinder.isMining() || bot.pathfinder.isBuilding()))
            return;
        try {
            bot.autoEat.eat();
        }
    ...
    bot.on('playerCollect', async (who) => {
        if (who.username !== bot.username || !bot.autoEat.options.checkOnItemPickup)
            return;
        try {
